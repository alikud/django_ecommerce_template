...
class JwtMiddleWare:
    ...
    def with_cookie(self, cookie_name = 'token', jwt_key = ""):
        """
            ...
            def container(request, *args, **kwargs):
                ...
                if cookie_name in cookies:
                    ...
                    try:
                        ...
                        payload = jwt.decode(cookie_name, settings.SECRET_KEY)
                        # payload some id
                        # get user from db
                        # auth it
                        # set session id into cookie if it is none
        ...
        def decorator(views_func):
            def container(request, *args, **kwargs):
                ...
                if header_name.lower() in headers:
                    ...
                    # if not valid will send 403 http status code
                    try:
                        jwt.decode(header_name, settings.SECRET_KEY)
                        return views_func(request, *args, **kwargs)
